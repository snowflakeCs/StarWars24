<template>
    <div>
        <v-container>
            <v-row>
                <v-col v-for="vehicle in vehicles" :key="vehicle.url" cols="12" sm="6" md="4" lg="3">
                    <v-card color="deep-purple-lighten-5" class="mx-auto mb-4" max-width="344">
                        <v-card-title class="textCool">{{ vehicle.name }}</v-card-title>
                        <v-card-subtitle class="mb-2">Modelo: {{ vehicle.model }}</v-card-subtitle>
                        <v-card-text class="text-caption">
                            <p>Costo: {{ vehicle.cost_in_credits }}</p>
                        </v-card-text>
                        <v-divider></v-divider>
                        <v-card-actions>
                            <v-btn block variant="flat" color="#680c7b" @click="showDetails(vehicle)">
                                Ver
                            </v-btn>
                        </v-card-actions>
                    </v-card>
...
export default {
    ...
    methods: {
        ...
        showDetails(vehicle) {
            this.$router.push({ name: 'VehiculoDetalle', params: { id: this.getIdFromUrl(vehicle.url) } });
        },
        getIdFromUrl(url) {
