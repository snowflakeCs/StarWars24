<template>
    <div>
        <v-container>
            <v-row>
                <v-col v-for="film in films" :key="film.url" cols="12" sm="6" md="4" lg="3">
                    <v-card color="deep-purple-lighten-5" class="mx-auto mb-4" max-width="344">
                        <v-card-title class="textCool">{{ film.title }}</v-card-title>
                        <v-card-subtitle class="mb-2">{{ film.director }}</v-card-subtitle>
                        <v-card-text class="text-caption">
                            <p>Episodio: {{ film.episode_id}}</p>
                        </v-card-text>
                        <v-divider></v-divider>
                        <v-card-actions>
                            <v-btn block variant="flat" color="#680c7b" @click="showDetails(film)">
                                Ver
                            </v-btn>
                        </v-card-actions>
                    </v-card>
...
export default {
    ...
    methods: {
        ...
        showDetails(film) {
            this.$router.push({ name: 'PeliculaDetalle', params: { id: this.getIdFromUrl(film.url) } });
        },
        getIdFromUrl(url) {
